...
switch ( $bps_version ) {
	...
	case ! strpos( $check_string, "BULLETPROOF $bps_version" ) && strpos( $check_string, "BPSQSE" ):
		...
		// .53.1: Create new block of Request Methods Filtered code & help text - not checking Custom Code
		if ( preg_match( $pattern18, $stringReplace, $matches ) ) {
...
// .52.6: fallback to mod_access_compat for everything else under the sun 
...
function bpsPro_apache_mod_directive_check() {
	...
	if ( current_user_can('manage_options') ) {
		...
		// end if ( esc_html($_SERVER['QUERY_STRING']) == 'page=bulletproof-security/admin/system-info/system-info.php' ) {
		// inpage BPS check, create db options and new htaccess files during BPS upgrade & Pre-Installation Wizard checks
		} else {
			...
			// .53: No time restriction for the Setup Wizard
